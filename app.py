...
def dashboard():
    ...
    total_eq = Equipment.query.filter_by(user_id=uid).count()
    pending = MaintenanceLog.query.join(Equipment).filter(
        Equipment.user_id == uid,
        MaintenanceLog.status != "Completed"
    ).count()
    completed = MaintenanceLog.query.join(Equipment).filter(
        Equipment.user_id == uid,
        MaintenanceLog.status == "Completed"
    ).count()
    critical = MaintenanceLog.query.join(Equipment).filter(
        Equipment.user_id == uid,
        MaintenanceLog.priority == "Critical"
    ).count()

    recent = MaintenanceLog.query.join(Equipment).filter(
        Equipment.user_id == uid
    ).order_by(MaintenanceLog.id.desc()).limit(5).all()
...
def reports():
    ...
    logs = MaintenanceLog.query.join(Equipment).filter(
        Equipment.user_id == uid
    ).order_by(MaintenanceLog.id.desc()).all()
